  ...
  "NonEeaBusinessAnswersHelper" when {
    ...
    "there are assets" must {
      ...
      "index headings correctly" in {
        ...
        result.head.rows.map(_.answer).contains(Html("Company outside UK and EEA")) mustBe true
        ...
        result(1).rows.map(_.answer).contains(Html("Company outside UK and EEA")) mustBe true
