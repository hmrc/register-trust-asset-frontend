...
class SharesOnStockExchangeViewSpec extends YesNoViewBehaviours {
  ...
  "SharesOnStockExchange view" must {
    ...
    behave like dynamicTitlePage(applyView(form), isTaxable = true, messageKeyPrefix, companyName, "hint")

    behave like pageWithBackLink(applyView(form))

    behave like yesNoPage(form, isTaxable = true, applyView, messageKeyPrefix, Seq(companyName))
