...
class AffinityGroupIdentifierActionSpec extends SpecBase {
  ...
  private val actionBuilder: DefaultActionBuilder = app.injector.instanceOf[DefaultActionBuilder]
  private val fakeAction: Action[AnyContent] = actionBuilder { _ => Results.Ok }
    ...
    "an Org user" when {
      ...
      "the user hasn't logged in" must {
        "redirect the user to log in " in {
          ...
          status(result) mustBe s"${frontendAppConfig.loginUrl}?continue=http%3A%2F%2Flocalhost%3A9781%2Ftrusts-registration&origin=register-trust-asset-frontend"
