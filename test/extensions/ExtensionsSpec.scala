...
class ExtensionsSpec extends SpecBase {
  ...
  "ListExtensions" when {
    "asSomeIf" must {
      ...
      "return None given a false condition" in {
        testList.asSomeIf(false) mustEqual None
      }

      "return Some(list) given a true condition" in {
        testList.asSomeIf(true) mustEqual Some(testList)
      }
