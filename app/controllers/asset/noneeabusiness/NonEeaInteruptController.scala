...
class NonEeaInteruptController @Inject()(
  ...
  private def actions(index: Int, draftId: String): ActionBuilder[RegistrationDataRequest, AnyContent] =
    identify andThen
      ...
      validateIndex(index, Assets) andThen
       requiredAnswer(RequiredAnswer(NamePage(index), routes.NonEeaInteruptController.onPageLoad(index, draftId)))

  def onPageLoad(index: Int, draftId: String): Action[AnyContent] = actions(index, draftId) { implicit request =>
    val name = request.userAnswers.get(NamePage(index)).get

    val preparedForm = request.userAnswers.get(NonEeaInterruptPage(index)) match {
      case None        => form
